...
{
    public final static int space = 0x20 ;
    public final static int unknown = 0x20 ;
    
    public final static String MacEncoding = "MacRoman" ; 
    public final static String DefaultEncoding = "ISO8859_1" ; 
    
    // should be changed into hashtable 
    protected static int translate(int mac_char) {
	switch (mac_char) {
    ...
    protected static byte translate(byte mac_char) {
	if (mac_char>0) return mac_char ;
	int unsigned = mac_char+256 ;
	int result = translate(unsigned);
	return((byte) (result-256));
    } // translate
    
     public static String fallBacktranslateASCII(byte[] mac_string) throws UnsupportedEncodingException {
	byte[] iso_string = new byte[mac_string.length] ;
	for (int i=0 ; i<mac_string.length ; i++)
	    iso_string[i]=translate(mac_string[i]);
	return new String(iso_string,DefaultEncoding);
    } // translateASCII

    public static String translateASCII(byte[] mac_string) throws QDException, UnsupportedEncodingException {
	try {
	    return new String(mac_string,MacEncoding) ;
	} catch (UnsupportedEncodingException no_mac_encoding) {
	    return fallBacktranslateASCII(mac_string); 
	}	
    } // translateASCII
