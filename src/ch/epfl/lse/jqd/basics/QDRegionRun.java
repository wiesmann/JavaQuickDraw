...
public class QDRegionRun 
{
    ...
    protected final static int END_MARK = 0x7FFF ;
    ...
    protected static QDRegionRun e_factory(DataInputStream stream, int y)
	throws IOException {
     ...
     */
    protected static Vector v_factory(DataInputStream stream, int size)
	throws IOException {
	int total = 0 ;
	Vector v = new Vector();
	while(true) {
	    ...
	    while(true) {
		final QDRegionRun run = e_factory(stream,y);  
		if (run==null) break ;
		v.addElement(run);
	    } // while
	}// while
	...
	throws IOException {
	Vector v = v_factory(stream,size);
	final int s = v.size();
