...
public class QDBitMap implements QDBitSource {
    ...
    public Rectangle getDestination() { return dest ; } 
	...
	return(2*theStream.RECT_SIZE+theStream.MODE_SIZE);
    } // readEnd
	...
	throws IOException, QDException {
	final int start_len=readStart(theStream);
	final int end_len =readEnd(theStream);
	final int data_len =readData(theStream);
	buildImage();
	return(start_len+end_len+data_len);
    } // read
	...
	ImageProducer temp = new FilteredImageSource(image_prod,f);
